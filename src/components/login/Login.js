...
const Login = () => {
  ...
  //         // setVerified=true;
  //         document.querySelector("label").textContent= result.user.phoneNumber +" welcome";
  //       }).catch((error)=>{
  //         prompt(error);
  //       })
  ...
  //     setFormError(null);
  //     try {
  //         await auth.createUserWithEmailAndPassword(remail, rpassword);
  ...
  //         ) {
  //             try {
  //                 await auth.signInWithEmailAndPassword(remail, rpassword);
  //                 setrPassword("");
  //                 setrEmail("");
  //             } catch (error) {
  ...
  const handleRegister = (e) => {
    ...
    auth.createUserWithEmailAndPassword(remail, rpassword).then((e) => {
      return db.collection("registeredusers").doc(e.user.uid).set({
        email: remail,
        pass: rpassword,
        //   uname:name,
        phone: phoneno,
    ...
    <Container>
      <LoginForm>
        <Logo>
          ...
          <pre>
            A Place to Share knowledge and better understand our College
          </pre>
          <h1>College Forum</h1>
        </Logo>
        <Form>
          <LeftSection>
            ...
            <Policy>
              By continuing you indicate that you agree to Forums Terms of
              Service and Privacy Policy.
            </Policy>
            {/* <LoginButton className="sign-in with otp"  onClick={signInWithotp}>Sign in with otp</LoginButton>
                        {/* <p>       Please login with RGIT registered phoneno</p> */}
            {/* <div id='recaptcha'></div>
                            <label></label> */}
            {/* <LoginButton type="submit" onClick={signInWithotp}>
                                OTP
                            </LoginButton> */}
          </LeftSection>
          <RightSection>
            ...
            />
            <Lable>Branch:CS/IT/MECH/INSTRU/CIVIL</Lable>
            <Input
              type="branch"
            ...
            />
            <Lable>OR</Lable>
            <Lable>SUBJECT:COA/DBMS/C..( ONLY FOR TEACHERS)</Lable>
            <Input
              type="subject"
        ...
        <Footer>
          <Languages>{/* <span>हिन्दी</span> <span>मराठी</span> */}</Languages>
          <About>
            <p>
              <span>About</span>・<span>Careers</span>・<span>Privacy</span>・
              <span>Terms</span>・<span>Contact</span>・<span>Languages</span>・
              <span>Developer</span>・<span>Press</span>・© College Forum 2022
            </p>
          </About>
        </Footer>
      </LoginForm>
...
const Container = styled.div`
  width: 100%;
  min-height: 117vh;
  background: url("assets/collegeimg.jpeg");
